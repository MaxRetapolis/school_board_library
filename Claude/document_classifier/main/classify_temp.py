...
logger = logging.getLogger('ClassifyTemp')
...
def classify_temp_files(max_files=None):
    """
    Classify files from the temp_inbound directory and move them to the appropriate output directories.
    ...
    # Get input files
    inbound_dir = os.path.join(project_root, 'Claude/temp_inbound')
    if not os.path.exists(inbound_dir):
        logger.error(f"Temp inbound directory not found: {inbound_dir}")
        return
...
def main():
    ...
    logger.info("Starting temporary classification process")
